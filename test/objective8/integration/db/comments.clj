...
(facts "about getting comments by uri"
       (against-background
        [(before :contents (do (ih/db-connection)
                               (ih/truncate-tables)))
         (after :facts (ih/truncate-tables))]
        (fact "gets the comments in reverse chronological order with aggregate votes and name of creator"
              (let [user (sh/store-a-user)
                ...
                (first (comments/get-comments draft-uri)) => (contains {:username (:username user)})
