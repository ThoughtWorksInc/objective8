        ...
        (fact "creates a draft when submitter id is a writer for the objective"
              (let [{objective-id :objective-id submitter-id :user-id} (sh/store-a-writer)
                    draft-data {:objective-id objective-id
                                ...
                                :content some-hiccup}
                    {response :response :as peridot-response} (p/request app (utils/api-path-for :api/post-draft :id objective-id)
                                                                         :request-method :post
        ...
        (fact "a draft is not created when submitter id is not a writer for the objective"
              (let [{objective-id :_id :as objective} (sh/store-an-objective)
                    ...
                    the-draft {:objective-id objective-id
                               :submitter-id submitter-id 
                               :content some-hiccup}
                    {response :response} (p/request app (utils/api-path-for :api/post-draft :id objective-id)
                                                    :request-method :post
                                                    :content-type "application/json"
       ...
       (against-background
        ...
        (fact "gets a draft for an objective with comment count included"
              (let [{objective-id :objective-id draft-id :_id :as stored-draft} (sh/store-a-draft)
                    ...
                    {response :response} (p/request app (utils/api-path-for :api/get-draft :id objective-id 
                                                                            :d-id draft-id))]
                (:status response) => 200
       ...
       (against-background
        ...
        (fact "gets drafts for an objective"
              (let [objective (sh/store-an-objective)
                    ...
                    {response :response} (p/request app (utils/api-path-for :api/get-drafts-for-objective :id (:_id objective)))]
                (:status response) => 200
       ...
       (against-background
        ...
        (fact "returns draft-id for previous draft"
              (let [objective (sh/store-an-objective)
                ...
                (get-in (p/request app (utils/api-path-for :api/get-draft
                                                           :id (:_id objective)
                                                           :d-id "latest"))
       ...
       (against-background
        ...
        (fact "gets section of draft"
              (let [{draft-id :_id objective-id :objective-id :as draft} (sh/store-a-draft {:content draft-content})] 
                (get-in (p/request app (utils/api-path-for :api/get-section
                                                           :id objective-id
                                                           :d-id draft-id
       ...
       (against-background
        ...
        (fact "gets sections of draft with annotation counts"
              (let [{draft-id :_id objective-id :objective-id :as draft} (sh/store-a-draft {:content draft-content}) 
                ...
                (get-in (p/request app (utils/api-path-for :api/get-sections
                                                           :id objective-id
                                                           :d-id draft-id))
       ...
       (against-background
        ...
        (fact "gets all draft annotations with the annotated section" 
              (let [{draft-id :_id objective-id :objective-id :as draft} (sh/store-a-draft {:content draft-content}) 
                ...
                (get-in (p/request app (utils/api-path-for :api/get-annotations
                                                           :id objective-id
                                                           :d-id draft-id))
